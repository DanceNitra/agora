# mnemo for Claude Code — deterministic coding memory that never resurrects a stale fact
...
- **Corrections stick.** You change an API signature, rename a symbol, move a file. mnemo keeps only the current
...
- **No LLM on capture.** Others summarize your session with an LLM, which drops facts, is non-reproducible, and
  cannot prove a value was erased. mnemo writes deterministically: same session, same memory, every time.
- **Provably erasable.** `mnemo` can delete a secret from every surface and prove it (hash-chain receipt). The
...
- **Zero dependency.** One pip package, no vector DB, no graph DB, no cloud. The store is a local JSON file at
  `<project>/.mnemo/coding_memory.json` that you can read, grep, or delete.
...
{
  "hooks": {
    "PostToolUse":      [{ "hooks": [{ "type": "command", "command": "python /path/to/mnemo_hooks.py" }] }],
    "UserPromptSubmit": [{ "hooks": [{ "type": "command", "command": "python /path/to/mnemo_hooks.py" }] }],
    "SessionStart":     [{ "hooks": [{ "type": "command", "command": "python /path/to/mnemo_hooks.py" }] }]
  }
}
```
...
#    mnemo injects ONLY:  auth.py :: current state -> def authenticate(token, scope): ...
...
lose facts, leak on deletion, and go non-deterministic. mnemo stays the correctable, auditable, zero-dependency
...
mnemo's integrity claims are measured against live mem0 and Graphiti, published with the harness next to every
