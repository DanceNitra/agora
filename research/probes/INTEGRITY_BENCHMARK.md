...
- It measures **integrity** (which version wins, can you undo it, can you prove erasure), **not recall quality**.
  We do **not** claim inspeximus retrieves better than mem0/Zep; on the one recall-adjacent cell here (conflict
  fidelity) inspeximus *ties* a naive verbatim baseline. A fair recall-accuracy comparison (LoCoMo / LongMemEval,
...
## Run it / add your system
    ...
    # free, local only:
    python research/probes/integrity_bench_revert.py --systems inspeximus

    # includes paid backends (needs OPENAI_API_KEY in server/.env; Graphiti needs a neo4j at bolt://localhost:7687):
    python research/probes/integrity_bench_revert.py --systems inspeximus,mem0,graphiti --n 20
...
## Cell 3 — conflict-consolidation fidelity  (`mab_official/run_inspeximus_official.py`)
...
| **inspeximus** (verbatim store) | **0.85** | keeps every fact; answerer resolves the final value |
...
*class* of memory, not about inspeximus's cleverness: **any verbatim store (inspeximus 0.85, naive 0.87, raw long-context
0.83) is ~5× mem0's 0.16**, because mem0's LLM extraction destroys facts when it summarizes big chunks, while a
verbatim store keeps them. inspeximus **ties the naive baseline** here — MAB's chunk-dump protocol hands inspeximus no
...
LLM-extraction memory ~5×"**, with inspeximus's keyed supersession as an additional integrity layer this particular
cell does not test. We publish the tie with naive next to the 5× so the frame can't be read as inspeximus-only.
...
|---|---|---|
| **inspeximus** (`forget_subject`) | **1.00** | nowhere — query 0, enumerate 0, history 0, raw 0 |
...
was erased cannot, on mem0, without extra scrubbing. inspeximus and Graphiti both erase across every surface. This
is a genuine capability difference (and the one honest place a competitor — Graphiti — ties inspeximus), not a "mem0
...
and a **real-entity** one (MemoryAgentBench pairs, where every system retrieves). inspeximus and naive are LLM-free;
mem0/Graphiti ran on gpt-4o-mini.

| system | poison removed from surface (real-entity, n=6) | (synthetic, n=12) | how it behaves |
|---|---|---|---|
| **inspeximus** (supersession + echo_guard) | **1.00** (6/6) | **1.00** (12/12) | retires A, echo_guard rejects the re-assertion → surface = `{B}` |
...
**Reading.** Only inspeximus removes the retired value from the retrieval surface: supersession retires A and
...
the reader to do the disambiguation; inspeximus does it in the store.

**Scope (no overclaim).** This is EXACT re-assertion poison (same key) — the classic echo/MINJA restatement.
Paraphrased poison that changes the key is a harder, separate case where inspeximus's exact-key echo_guard is weaker;
we do not claim that here.

## The landscape — is inspeximus unique among the *top* systems, or only vs mem0/Graphiti?

The cells above measure three systems live. The fair question is whether inspeximus's combination is special across the
...
is a code-reading, not a live benchmark — only inspeximus / mem0 / Graphiti rows are measured above). Markers: `conf` =
...
|---|---|---|---|---|---|---|
| **inspeximus** | **YES** | **YES** | **YES** (all surfaces, measured) | **YES** (measured) | **YES** | deterministic key supersession + echo_guard |
...
So the honest, sourced claim is **not** "inspeximus beats everyone" and **not** a bare "ONLY": it is that **across this
11-system survey (3 live-measured, 8 read from docs), inspeximus is the only one that combines all of these integrity
...
integrity is the corner inspeximus occupies, not a total-ranking crown. The mechanisms are textbook (AGM belief
...
+ a soft speaker prefilter via `recall(prefer=...)`; see `inspeximus/examples/recall_recipe_locomo.py`).
...
| metric (inspeximus, tuned recipe, k=25) | value | n |
|---|---|---|
...
**Honest scope.** This is inspeximus's **own** number, not a head-to-head: mem0/Zep publish LLM-judged end-to-end QA
...
claim is narrow and checkable: **inspeximus retrieves a supporting turn for 78% of LOCOMO questions, deterministically
and for free — run `research/probes/retrieval_recall_locomo.py` and reproduce it.**

## Planned cells (harness shape is the same)

- **atomic-keyed conflict** — the contract where inspeximus's supersession IS exercised (the coding-agent plugin
...
section above: inspeximus retrieval-recall@25 = 0.78 any / 0.65 all, LLM-free); we still make no cross-system recall
