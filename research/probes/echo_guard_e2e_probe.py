...
Needs echo_attack_paraphrases.json. RUN: python research/probes/echo_guard_e2e_probe.py
"""
...
para = json.load(open("research/probes/echo_attack_paraphrases.json", encoding="utf-8"))
...
def run(guard, echo_kind):
    ...
    for c in cases:
        ...
        m = Inspeximus(path=path); m.echo_guard = guard
        key = "fact:" + cid(c)
        for t in texts:
...
json.dump(out, open("research/probes/echo_guard_e2e_probe_result.json", "w"), indent=2)
