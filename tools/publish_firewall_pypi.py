"""
publish_firewall_pypi.py - build + upload grounding-firewall to PyPI.
Reads PYPI_TOKEN from server/.env; passes it to twine via env vars only (never on a command line or in
printed output). Re-run after bumping the version for a new release.

Usage:  python tools/publish_firewall_pypi.py
"""
...
PKG = os.path.join(ROOT, "grounding-meter", "firewall_pypi")
...
def main():
    ...
    print("\nLIVE: https://pypi.org/project/grounding-firewall/   ->   pip install grounding-firewall")
