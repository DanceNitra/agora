...
has no provenance/currency mechanism at recall. mnemo's deterministic, zero-LLM **`recall(trusted_only=True)`**
...
**2. mnemo's defense holds (`agora_output/lab/poison_defense_working.py`, n=100, adaptive attacker):**
...
**3. Zero-dep demo anyone can run (`examples/integrity_recall_demo.py`, `pip install mnemo`):** stale/rolled-back/poisoned
vs mnemo current — lexical, no embedder/API key.
...
- mem0 was run with a LOCAL LLM (qwen2.5:7b), not its native gpt-4o-mini. The **poison failure is architectural**
  ...
  might collapse the stale fact — stated as a caveat, not hidden). Single trial for the mem0 demo; n=100 for the mnemo
  defense.
- mnemo's `trusted_only` is **high-friction by design**: only facts you ANCHOR with a trusted signature are protected;
...
1. r/RAG / r/LocalLLaMA practitioner post (disclose "I build mnemo (OSS)" upfront; end on a real question).
