...
provenance (inspeximus retract_lineage + verify_writes) the UNIQUE policy whose audit trail survives?
...
B inspeximus retract_lineage+verify_writes); measure "stale-derived-active rate" per policy + bootstrap CI on the gap.
...
DEMM-style rubric with a ground-truth-blind judge. inspeximus self-scores <8/8 (fails authority-binding, decision-basis,
external-anchorability — its own governance_report docstring concedes forge-able tombstones = self-incriminating,
shareable).
First step: run one correction lifecycle through inspeximus, capture receipt bytes, blind-judge 8 questions (k/8).
...
writes, and is it a property of the PRODUCT or the control-plane placement? Hold inspeximus fixed, move only the
...
CONVERT into a confident downstream fabrication, at what rate + latency, and does inspeximus's integrity layer actually
...
Why exp: converts our one irreplicable asset into a standing measured falsifier; intervention arm IS inspeximus.
...
DOES bitemporal invalidation so "inspeximus unique" collapses; and we already killed this on 2026-07-12
...
with a deterministic runnable falsifier this week, binds to inspeximus's real differentiator, self-refilling ledger).
