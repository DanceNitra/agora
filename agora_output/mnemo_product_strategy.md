# mnemo → #1 agent-memory product — capability & strategy sketch

> Drafted 2026-07-17 from a 5-lens market scan (competitive teardown · product/cyber security threat model ·
> branding/category · distribution · monetization), cross-referenced against mnemo's actual shipped API surface
...
One line: *mnemo — the self-correcting memory layer for AI agents. Memory your agent can audit.*
...
field's numbers are non-reproducible). mnemo's edge is the axis they all fake: **deterministic, auditable,
...
| Capability | mem0 | Zep | Letta | Cognee | Supermem | Redis | **mnemo** |
|---|---|---|---|---|---|---|---|
...
**Top-5 ownable gaps** (underserved × mnemo-fit): (1) deterministic reproducible correction, (2) built-in
...
long-context 100 · mnemo 97 · naive-verbatim 96 · **mem0 88** · random-drop 67. CRITICAL: mem0 scored **88%**
on our clean per-fact harness, NOT its published 18% (that number is a default-pipeline/raw-conversation
artifact). So the CR-accuracy gap is REAL but MODEST — mnemo 97 vs mem0 88 = 9pt, the measured cost of mem0's
...
- **Article-12 tamper-evident audit chain with redaction-at-write** (reconciles EU-AI-Act "retain" vs GDPR
  ...
  `erasure_report`, receipts). → A written mapping mnemo-receipts → Article 12(2) clauses + a runnable
...
- **Brand = `mnemo`. Canonical install slug = `agora-mnemo`, always shown identically everywhere** (PyPI has no
  namespaces; the split is fine if the snippet is canonical). Keep `import mnemo` / `mnemo` CLI as the pretty name.
- **Secure `mnemo.ai` / `getmnemo.com`**, 301 to docs (Pinecone/Turso pattern); a live `mnemo.dev` we don't own
...
where mnemo loses**, on the **integrity dimension** incumbents fake. Lead with LongMemEval/MemoryAgentBench.
...
- **Domains:** decision + purchase of `mnemo.ai` / `getmnemo.com` (owner action).
