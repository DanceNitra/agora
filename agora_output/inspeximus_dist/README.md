...
`pip install inspeximus` · [PyPI](https://pypi.org/project/inspeximus/) · [Hugging Face](https://huggingface.co/Danchi17/inspeximus) · [DOI 10.5281/zenodo.21128549](https://doi.org/10.5281/zenodo.21128549) · [Homepage](https://dancenitra.github.io/inspeximus/) · MIT · v1.1.0
...
- **`DeletionManifest`** (`inspeximus.deletion_manifest`) — a **cross-store** erasure record: register every place a
...
[`research/probes/retraction_propagation.py`](https://github.com/DanceNitra/agora/blob/main/research/probes/retraction_propagation.py).
...
vector index. Receipt: `research/probes/continuous_chunk_recall_probe.py`.
...
legacy). Receipt: `research/probes/legible_warrant_scoped_budget_probe.py`.
...
byte-identical legacy path. Receipt: `research/probes/spend_irreversible_require_earned_probe.py`.
...
`research/probes/membench_recall_probe_v2.py`). `tie_recent=eps` re-orders candidates whose relevance is within
...
`research/probes/membench_recency_tiebreak_probe.py`): `tie_recent=0.05` on centered cosine cuts stale-beats-fresh
...
(`research/probes/echo_attack_probe_v2.py`, retrieval-level stale-answer-rate, 43 corrected-fact cases; echoes
...
`research/probes/retrieval_exposure_coverage_probe.py`). That gap is an **attribution** problem, not a ceiling:
...
Measured (`research/probes/outcome_propagation_probe.py`): conversion lifts from manual-attribution-limited to
...
(`research/probes/revert_by_reference_probe.py`, resistance **0.00 → 1.00**). Discrimination gap 1.0 vs a
...
and preserves `as_of`). Default `None` = unbounded legacy, byte-identical. (`research/probes/` Lab 29992a.)
...
contribute **zero** trusted witnesses (`research/probes/seed_anchored_trust_probe.py`, 4/4). Default empty set =
...
decision changes (`research/probes/supersession_policy_stamp_probe.py`, 10/10).
...
tamper-evident logs. Receipt: `research/probes/forget_subject_tombstone_probe.py` (8/8).
...
`research/probes/inspeximus_session_adapter_probe.py` (11/11). Adapters live under `inspeximus.integrations` (opt-in extras).
...
Verified end-to-end against the real `autogen-core` (`research/probes/inspeximus_autogen_adapter_probe.py`, 7/7,
...
Verified end-to-end against real `langgraph` (`research/probes/inspeximus_langgraph_adapter_probe.py`, 9/9, incl. the
...
`research/probes/check_conflict_probe.py` (8/8).
...
(`research/probes/inspeximus_llamaindex_adapter_probe.py`, 4/4, incl. "corrected value not re-injected"). Subclasses
...
2.4.0 (`research/probes/inspeximus_adk_adapter_probe.py`, 4/4, incl. per-user isolation, current-truth, and
...
(`research/probes/inspeximus_pydantic_ai_adapter_probe.py`). Importing this module imports Pydantic AI (opt-in
...
Receipt: `research/probes/extractor_hook_probe.py` (7/7).
...
Verified against real `autogen-core` (`research/probes/extractor_adapter_wireup_probe.py`): without the extractor
...
`research/probes/retention_probe.py` (7/7, incl. "current keyed value and semantic facts are never expired").
...
Measured (`research/probes/route_probe.py`, 148 rows): every *marked* class — corrections, value-obscuring
...
stolen private key or authenticate a human. Adversarial receipt: `research/probes/authorized_revert_probe.py`
