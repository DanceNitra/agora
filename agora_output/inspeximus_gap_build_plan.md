# inspeximus gap → build plan: dominate every top-10 agent-memory product
...
> 9 Hindsight · 10 Second Me), each compared to inspeximus. Goal: build the CONVENIENCE features they have, while
> keeping inspeximus's unique moat (deterministic corrections, verifiable forgetting, poisoning defense, zero-dep,
> auditability) that NONE of them have. Result = the only memory that is BOTH as convenient AND has the integrity moat.

## The convergent gaps (what the leaders have, inspeximus lacks) — compatible with our principles

| gap | who has it | compatible with inspeximus (zero-dep / deterministic / no-LLM-on-write)? | priority |
|---|---|---|---|
| **Automatic lifecycle-hook capture** (no manual `remember()`) | **Claude-Mem #1, agentmemory #6** (both top coding-agent memories) | ✅ hooks call inspeximus's deterministic write API, no LLM | **1 (the wedge)** |
...
| **Zero-LLM entity-link / lightweight graph + multi-hop** | **gbrain #5** (self-wiring, ZERO-LLM extraction), agentmemory, Hindsight | ✅ gbrain proves a deterministic no-LLM graph is possible; inspeximus already has bitemporal as_of | **3** |
...
Ship an inspeximus Claude-Code plugin: lifecycle hooks (SessionStart / PostToolUse / Stop / SessionEnd) that
auto-capture tool events into inspeximus's deterministic store, `claude mcp add` one-liner, `<private>` redaction.
inspeximus then matches their convenience AND adds what they cannot: corrections that stick, verifiable forgetting,
...
inspeximus builds every convenience the top-10 have (auto-capture, hybrid recall, viewer, graph) using ONLY
