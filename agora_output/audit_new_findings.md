...
- **Frontier question — does confidence-weighting survive miscalibration?** The confidence-weighted soft
  filter (`w = extractor_confidence x filter_selectivity` as the RRF fusion weight, `research/probes/
...
## Frontier candidate from inbox triage (2026-07-01): inspeximus poison-guard vs. AgentPoison-style attack

- **Frontier question:** does inspeximus's corroboration-gate (episodic->semantic promotion requiring earned
  trust, [[inspeximus-poison-guard-hole]]) hold up against an AgentPoison-style OPTIMIZED trigger attack — not
...
- **Why deferred, not built now:** a proper test requires (a) implementing or approximating the
  ...
  gate since this is a security claim about our own shipped product (inspeximus). This is a full audit-post-
...
## AgentPoison vs inspeximus — TESTED (2026-07-01), supersedes the earlier "deferred frontier flag"

Ran a REAL gradient-guided (HotFlip) AgentPoison-style attack (arXiv:2407.12784) against inspeximus's
semantic-retrieval channel, using a differentiable dense retriever (all-MiniLM-L6-v2) as inspeximus's
embedder so attack and defense share one embedding space. Artifacts (runnable receipts):
research/probes/agentpoison_hotflip_probe.py (+_result.json), agentpoison_dilution_check.py,
...
- inspeximus's existing poison-guard (episodic->semantic graduation, corroboration-gated) is IRRELEVANT to
...
research/probes/agentpoison_defense_check.py, agentpoison_setcoherence_proto.py, agentpoison_softcoherence_check.py).
...
- Defense #1 — stored-memory ISOLATION outlier (flag a memory whose nearest-neighbor cosine to the store
  ...
  corpus similarity. Reverted from inspeximus core (do not ship a non-working guard).
...
promising OPT-IN adversarial-mode feature but is NOT promoted to inspeximus core yet (validated only on n=8/10,
...
benign). Artifacts: research/probes/agentpoison_multiretriever_check.py (+_result.json),
...
mean. inspeximus's dense-retrieval memory is broadly poisonable, robust to dilution, across embedder families.
...
- inspeximus's anisotropy CENTERING does NOT rescue it -- it DESTROYS the separation on ALL three (MiniLM
...
universal inspeximus defense") is honestly KILLED. Honest product contributions instead: (a) a inspeximus README
...
research/probes/agentpoison_coherence_attack.py (gpt2 as coherence surrogate; ppl gate = 1000).
...
empirical result grounded in Ethayarajh 2019's general anisotropy finding, not that paper.) Artifacts: research/probes/agentpoison_influence_gate.py (+_result.json),
...
anisotropic encoders). The layer that GENERALIZES is influence-gating by corroboration: reuse inspeximus's
...
art) that clears the RAISED BAR. REVIVES Track A: the influence-gate is a shippable, generalizing inspeximus
opt-in feature (unlike set-coherence which failed to generalize). Next: promote to inspeximus core (opt-in,
...
ARTIFACT (now public + linked from the post): research/probes/correlated_sources_calibration.py
...
degradation curve): research/probes/robustness_filter_independence.py, re-run this cycle.
...
research/probes/grounding_firewall_cloud_poison.py (strong-model table),
research/probes/grounding_firewall_hardened.py (saturated-prior confound + signed-margin baseline
+ clean/poison arms).
...
research/probes/grounding_meter_v3_analyze.py (+_gm_v3_sweep.py + _gm_v3_raw.json).
...
6 sources asserting the FALSE option): research/probes/grounding_meter_strong_models.py --
...
J=1.2/2.0/3.0/4.0). Public probe promoted: research/probes/governance_hysteresis_ising.py.
...
- **Crucible ledger fix:** post was styled "A Crucible entry / FAILED" but was NOT in crucible.json (71 entries). ADDED a machine-readable FAILED entry to server/.replications.json + re-rendered (render_crucible.py) -> now 43R/15F/15NC, entry live in public crucible.json + index.html, so the "Crucible entry" claim is now backed. Model+date of the reader run were NOT logged (reproducibility gap, disclosed). Promoted+linked probe research/probes/ragdead (generator + independent gold-re-derivation scorer + result JSON + gold).

## Audit #29 — memory-poison-resistance-measured (2026-07-04, MINOR-FIX/REFRAME-lite)
- **Frontier question (practitioner + blind-spot, strong):** DECAY IS THE WRONG TOOL for the threat that matters. A poisoned entry fires on the NEXT matching query (immediate), and real attackers re-pump (continuous) -> a ~3-week fade is useless against a live attacker. The right levers are read-time INFLUENCE-GATING (inspeximus's own recall(influence_only=True) -- gate what drives an ACTION, not what's stored) + monitor/slash/spend_irreversible (the jackson-thread source-keyed stack). This post's decay-fade is only a hit-and-run BLAST-RADIUS CAP. Also: most legit high-value facts arrive ONCE from a single source ("my flight's Tuesday"), so a >=2-source/outcome corroboration gate adds write-friction to 99% honest single-shot writes to slow the 1% poison -> co-ship, not primary control. Practitioner receipts: Mem0 gates writes on salience/consistency (not corroboration); Zep recency-wins HELPS the attacker; SMSR (write-time HMAC provenance + certified eviction) + OWASP Agent Memory Guard are the shipped runtime defenses.
- **Method lesson (parameter readout, again):** "~21 days" is d* = tau_half * log2(V_pump/V_truth) = 7*log2(8) = 21 = ~3 episodic half-lives -- a PARAMETER READOUT of (pump 8, threshold 5, 7d half-life), not a discovered constant; the probe's docstring PREDICTS "2-3 weeks" before running. The OLD-vs-NEW 100%-vs-0% gap is ARITHMETIC once you fix which decay tier the poison lands in (180d vs 7d) -- a sanity check that (graduation gate) + (two-tier decay), BOTH prior work, compose as expected, NOT an independent empirical property. Same parameter-readout pattern as governance-hysteresis + verification-tax. Report the INVARIANT (formula), not the point.
- **n discipline:** fade curve n=40, sustained-attacker table n=15 (post said "40 scenarios", didn't flag) -> fixed. Added Wilson CIs: day-14 52% [37,67], day-21 0% [0,9], sustained-once 39% [20,64], every-30d 72% [48,89].
- **Prior-art (all CONFIRMED):** AgentPoison (Chen 2024, 2407.12784, NeurIPS 2024), MINJA (2503.03704), OWASP ASI06 = "Memory & Context Poisoning" (full title, cites AgentPoison+MINJA), BEAM (2510.27246, ICLR 2026 -- real, the flagged risk held), LongMemEval (2410.10813), LoCoMo -- none score poison durability. Scoped "no benchmark" to those three. Promoted+linked probe research/probes/poison_durability.

## Audit #30 — adaptation-corruption-separation-law (2026-07-04, REFRAME)
- **The "law" is a classical result under four names (unanimous 5-lens + storm).** The coupling = Grossberg's stability-plasticity dilemma (1980); the fast+slow-gated ESCAPE = Complementary Learning Systems (McClelland, McNaughton & O'Reilly 1995 -- fast hippocampus + slow neocortex + consistency gate); the two-tier eviction = the ARC cache (Megiddo & Modha 2003, shipped in ZFS); the B<d boundary = transient/finite-duration change detection (Guepie, Fillatre & Nikiforov 2012); the latency floor = CUSUM optimality (Page 1954 introduced; Lorden 1971 ASYMPTOTIC; **Moustakides 1986 EXACT** -- post credited only Lorden/Page). Residual-ours: the cross-mechanism catalog + a pre-registered trust instance + the NAB-16 asymmetry. REFRAME applied: title "hidden law"->"classical tradeoff", named-law callout->"adaptation-corruption coupling (Grossberg) ... the CLS escape", "the only escape is architectural"->"the known escape ... same design neuroscience calls CLS", "we've shipped this escape"->"shipped this classical architecture ... our contribution is measuring", credited CLS/ART/ARC/Guepie/Moustakides in body+footer+prior-art list, EN+SK.
- **FRONTIER QUESTION (blind-spot 6th lens, strong -> next Lab):** every result assumes a DETECTOR-OBLIVIOUS adversary. "Escape iff B<d" hands a detector-AWARE attacker the win condition: set B=d-eps, drip below the persistence threshold forever; and d LEAKS (observe which writes consolidate -> binary-search d in O(log d) probes). So the true quantity is not a fixed latency floor but a STACKELBERG equilibrium value (defender commits to a policy over d, attacker best-responds). Testable next cycle (reuse the trust/CUSUM harness): iterated ~200-round game, defender = 2-channel with 4 d-policies {fixed-secret, fixed-leaked, uniform-random d, bandit-learned d}, attacker = gradient-free best-responder tuning (B,magnitude,spacing) on accept/quarantine outcomes. Falsifiable: fixed-d (secret OR leaked) collapses to ~single-channel; RANDOMIZED d retains >=50% of the oblivious advantage; bandit-d in between + probe-trainable. If fixed-d collapses -> reframe "irreducible floor" to "floor vs oblivious corruption; equilibrium value vs adaptive corruption" (a better inspeximus gate claim). Sub-frontier B: online/meta-adaptive d (bandit-over-detectors) is ITSELF a single rule on a shared stream -> the coupling may RECURSE at the meta level; does randomized-d break the recursion?
- **Method lessons (this audit):** (1) VALIDATE caught a mislabeled multiplier -- "22.3 at a 30x spike" is actually the x150 row (heavy-tailed unbounded sweep 5->150; at x15 EWMA is only 2.28) -> fixed to 150x. (2) The regime sweep FAILS its own pre-registered T1 (min ratio>1.1): at the easy corner (mu0=0.95,p_bad=0.4) naive/CUSUM=0.92, i.e. naive is MARGINALLY BETTER -> "shrinks to ~0" reframed to "REVERSES (~0.92)". (3) NAB min-false-alarm-EVENTS metric favors the accumulating detector by construction (CUSUM exceedance merges into 1 event; point detector fragments into many) + degenerate streams (35M x divide-by-tiny-scale shift; nyc_taxi naive=inf) inflate the asymmetry -> caveated, kept only the one-sided sustained=>persistence asymmetry. (4) best-of-N is a Procrustean 4th pillar (no time-series channel; "escape"=cap N~1/h) -> labeled a selection-pressure ANALOG, not a temporal instance. (5) "information-theoretically irreducible" leaned on undetectable-backdoors (Goldwasser 2022, which is COMPUTATIONAL) -> re-anchored to the transient missed-detection bound (Guepie 2012). Promoted+linked public probe research/probes/separation_law/ (separation_law.py reproduces A trust/B CUSUM-redteam/C boundary/D regime-reversal/E consolidation-150x; nab_asymmetry.py reads a public NAB checkout). Storm briefing: storm-reports/adaptation-corruption-separation-law-briefing.html (grade B).

## Audit #36 — rag-supersession-blind-spot (2026-07-04, REFRAME; prepped for r/RAG share)
- **Citation HARD-verified (the make-or-break):** arXiv:2606.26511 is REAL — Neeraj Yadav, "Temporal Validity in Retrieval Memory: Eliminating Stale-Fact Errors...", MemStrata, June-2026 PREPRINT. Abstract verbatim: cosine AUROC 0.59 near-chance, "contradictions often more embedding-similar to the original than rephrased duplicates", "RAG serves superseded values 15-40% of the time", "a deterministic (subject, relation, object) supersession rule retires the stale value in a bi-temporal ledger... MemStrata drives this to ~0%". CONFIRMED twice (my WebFetch + verify-claims agent).
- **THE overclaim the gate killed:** the post framed inspeximus's key= as "the STORAGE-side complement to Yadav's RETRIEVAL-side result... the two halves want to be designed together" — MANUFACTURED. Yadav's OWN paper proposes the deterministic SRO retire-on-write rule -> ~0%; it is not retrieval-side, it IS the storage mechanism. Our real contribution = independent replication receipt + open-source implementation of HIS fix, credited. REFRAME applied all surfaces EN+SK: title "Why RAG serves stale facts"->"Why SIMILARITY-ONLY RAG..."; killed complement framing; "Yadav names+measures; here's OUR fix"->"Yadav names the blind spot AND proposes the fix, both halves his; inspeximus = runnable implementation, not the idea"; added a SCOPE callout (production systems already fix via upsert/metadata/bi-temporal; the narrow point is the cosine SIGNAL is time-blind); credited STS-vs-NLI negation-insensitivity (the observation) + SCD Type 2 / bi-temporal (Snodgrass, Kimball) / Zep-Graphiti (the fix). Preprint labeled.
- **VALIDATE (probe re-run, research/probes/supersession_replication.py, local nomic):** every number reproduces EXACTLY — cos(orig,contradiction)=0.768, cos(orig,rephrase)=0.843, 10/24, AUROC 0.613, stale 41.7%->0.0%. Method caveats added: AUROC 0.613 at n=24 has 95% CI ~[0.42,0.81] STRADDLING chance -> "direction reproduces, not the value; both ours 0.613 and his 0.59 indistinguishable from 0.5"; 41.7% = 10/24 Wilson ~[24%,61%] (dropped any "above Yadav's range"); 0% is DEFINITIONAL (upsert-by-key never serves old value) -> flagged, interesting number is the ~40% not the 0%; the near-chance AUROC is partly by stimulus design (minimal-edit contradictions vs surface rephrases = the STS-vs-NLI gap) -> conceded as a demonstration not a discovered rate.
- **Re-audit caught (both skeptics + self-test):** SK TLDR was NOT reframed (still "Reprodukovali sme to a odmerali deterministickú opravu" = implied WE did both halves; no Yadav/MemStrata) -> fixed to mirror EN (credits Yadav, adds the contradiction>rephrase hook). Everything else clean; LLM-summary self-test extracts the credited-replication framing unambiguously (4+ credit surfaces).
- **r/RAG share draft (gated, owner posts):** agora_output/drafts/20260704_rRAG_supersession_blindspot_DRAFT.md. Dunk-proof design: leads with the counterintuitive TRUE bit (contradiction MORE cosine-similar than a rephrase), credits Yadav in TITLE + first line + invites him to correct, pre-empts the 4 standard objections (just-upsert/SCD2; strawman-baseline; n=24-synthetic; contradictions-obvious) inline. Text self-post, r/RAG. Method lesson: on a REPLICATION post, the landmine is (1) is the cited paper real (verify HARD before amplifying) and (2) does the cited paper already contain OUR "fix" (it did) -> credit the fix as theirs, frame ours as implementation.

## Audit #31 — agent-memory-eviction-two-tier (2026-07-04, REFRAME -> deployed)
- **VALIDATE (public probe research/probes/eviction_twotier.py re-run, cap=150, 15 seeds):** every post number reproduces exactly — LRU 0.80/0.13/0.00, LFU 0.14/0.11/0.00, value 0.22/0.65/0.57, two-tier(f=30%) 0.81/0.64/0.57. Confirmed the tee-up caution: "universal" holds ONLY at tt30 (protected fraction=30%); tt15 FAILS (rare 0.44, flood 0.42). The universality check uses a 0.02 tolerance and two-tier is 0.641 vs value's 0.646 on rare-critical — a tiny REGRESSION, so "no regression" was false. Probe was NOT linked from the post (standing-gate violation) -> added.
- **THE severe-test the STORM skeptic demanded (and it reshaped the piece):** ran ARC (Megiddo-Modha 2003, the SOTA self-tuning recency/frequency hybrid) as a baseline on the same 3 workloads. ARC matches on locality (0.80) but FAILS rare-critical (0.12) and the wide flood (0.00) — same as plain LRU — because it infers importance from observed REUSE, and neither regime supplies a reuse signal (rare items are seldom-accessed; the flood is wider than cache+ghost). Folded ARC into the public probe.
- **Oracle-leakage objection (steelman+method, called fatal) DEFEATED by measurement:** replaced the offline value_tag (= total future access-weight) with a strictly WRITE-TIME estimator (running max of observed per-access importance, zero before first sight). Result nearly identical (0.805/0.640/0.569 vs oracle 0.805/0.641/0.569) and still crushes ARC -> the win is OBSERVED IMPORTANCE (a signal agents realistically have), not future knowledge. Value-noise sweep: degrades gracefully, still >2x ARC on rare-critical at sigma=3 (a poor estimate) -> needs a signal better-than-noise, not a calibrated oracle.
- **Prior-art (STORM 5-lens + 4 citation verifiers, all vs PRIMARY, CONFIRMED):** "no single online rule is universal" = textbook competitive analysis (Sleator-Tarjan 1985 k-competitive; Belady 1966 offline-optimal). Two-tier structure = segmented caching SLRU (Karedla 1994) / 2Q (Johnson-Shasha 1994) / LRU-K (O'Neil 1993) / ARC (2003, self-tunes the split we fixed by hand; shipped ZFS/IBM/briefly-PostgreSQL, removed over IBM patent). "Value-aware beats access-pattern when value is decorrelated from frequency" = cost-aware caching GreedyDual-Size (Cao-Irani 1997) / GDSF (Cherkasova 1998, NOT Dilley — attribution fixed), learned variants EVA/Hawkeye/LeCaR/CACHEUS. Mem0 already recommends "LRU with a salience floor" (verified vs blog).
- **REFRAME applied EN+SK:** headline "No single rule wins - but two tiers do" -> "Even a self-tuning cache misses what matters"; added ARC row + finding; scoped "universal" to tuned-f-vs-single-classic-rules; killed "no regression"; foregrounded value-admission (with observable estimator) as the real lever; credited all prior art in body+footer; linked probe (EN+SK); fixed metric definition (access-WEIGHT not raw accesses); "same as LRU" -> "no better than LRU" (LRU is 0.13). Not a Crucible entry (piece).
- **FRONTIER Q (blind-spot lens, genuinely novel + testable next Lab):** does explicit BOUNDED-CAPACITY tiering beat a value-weighted SINGLE policy (GDSF / cost-aware ARC) only under ADVERSARIAL value? Hypothesis: tiering is irrelevant under noisy-but-honest importance but strictly dominant under a POISONED importance signal, because bounded protected capacity caps the poisoning gain -> reframes "tiers protect rare items" to "tiering is an adversarial-value containment bound." The protected tier itself is an attack surface (inflate junk value to seize it).
- **Method lesson:** same relabel pattern as #24/#25/#28/#29/#30 — a cross-domain "universal architecture" that recovers no new theorem is textbook caching; the value came from (a) adding the SOTA ARC baseline the original omitted, and (b) killing the oracle-leakage objection with an observable-estimator re-run. Never publish a "beats the classics" eviction claim without ARC (or a cost-aware baseline) in the table.

## Audit #32 (best-of-n-exploitability) — 2026-07-05
- **Frontier Q (estimator gap, recurring):** the whole practical value of "cap N" hinges on estimating the exploitable-tail rate / proxy-vs-gold gap CONTENT-BLIND (ensemble disagreement, a held-out unselected verifier, self-consistency variance climbing with rank). This is the SAME estimator-gap pattern flagged in verification-tax (#25) and the grounding posts — a candidate cross-post program: a content-blind over-optimization detector that fires before a gold judge would.
- **Method lesson (VALIDATE before over-correcting — again):** the stress-claim skeptics claimed best-of-N's argmax is an extreme-value operator that manufactures bias from PURE unbiased noise (winner's curse → accuracy collapse). The probe REFUTED it: with unbounded Gaussian noise INDEPENDENT of correctness, accuracy stays monotone→1.0 (the fixed signal gap dominates the √log N extreme-value spread; the curse cancels symmetrically). Same lesson as diversity-flip — a plausible auditor critique must itself be validated before you "fix" the post toward it. The real correction was subtler: exploitability = error CORRELATED with wrongness (continuous), not noise magnitude.
- **The continuous-exploitability knob** (probe case A''): a biased verifier with NO discrete "scored-above-every-correct" tail still collapses once wrong-but-plausible answers can merely OUT-score correct ones (b>1). The discrete hack tail is the extreme; this bridges Case A and Case B and matches Gao 2022's imperfect RM + Eisenstein 2023's shared-error axis.
- **Prior-art bank for the BoN/overoptimization post family:** Gao-Schulman-Hilton 2022 (2210.10760, √KL parabola → finite optimal N); Khalaf et al. 2025 (2506.19248, NeurIPS Spotlight, proves rise-then-fall inevitable + HedgeTune finds optimal N + Best-of-Poisson); Eisenstein et al. 2023 (2312.09244, COLM, shared-vs-idiosyncratic); Coste et al. 2023 (2310.02743, ICLR, ensembles); Beirami et al. 2024 (2401.01879, ICML, KL upper-bound + win-rate ≤ n/(n+1)). NOTE: the circulating "25% label noise → BoN degrades past N>16" figure is UNVERIFIED (not in 2503.21878) — do not cite.

## Audit #33 (ai-memory-consolidation-scope-law) — 2026-07-05
- **Near-duplicate pattern (editorial):** #33 is the weaker sibling of #30 (adaptation-corruption-separation-law) — same coupling→two-channel→latency-floor family, same CUSUM/CLS anchors. Owner chose REFRAME-standalone (not collapse like #5→#2) because #33 has a distinct keeper: the measured negative result "don't gate bounded embedding recall" (a tuned EWMA dominates the gate on real nomic embeddings). Lesson: when two posts share a mechanism family, the later one must credit the shared textbook + cross-ref the sibling + lead with its differentiator, not re-claim the shared part.
- **"Irreducible in information" was FALSE (fixed):** a fixed-d persistence selector's latency floor is the cost of a WEAK detector, not an information bound — a sequential likelihood-ratio/CUSUM test reaches the same confidence in fewer steps. Lorden 1971 lower-bounds the delay >0 (some latency is irreducible) but does NOT license the specific d. Standing check: never call a specific selector's cost an "information bound" without computing the minimax quantity.
- **Definitional-scope-law pattern (recurring):** "gate helps iff magnitude unbounded" = "bounded input → bounded breakdown for any operator" = the boundedness hypothesis of the robustness theorem, relabeled. The EMPIRICAL direction (EWMA dominates the gate on real embeddings) is the keeper, not the "law". Same definitional-readout pattern as #24/#25/#28/#29/#30/#32.
- **Adaptive-attacker → provenance (cross-post convergence):** a fixed-d confirmation delay is defeated by a patient attacker who drips + shortens the burst (MINJA). Against an opponent who sees the threshold, no content-blind timing rule is robust — the invariant lever is provenance/source-identity. This is the SAME conclusion as the Veracity-Gap post (external-verified-identity); the two now cross-link. Frontier Q: can any content-blind detector bound worst-case poison against an adaptive adversary at all, or is provenance the only lever?
- **Method lesson (VALIDATE caught a self-contradiction):** the post claimed "the coupling holds in both regimes" but its own real-nomic lab returned coupling-monotone=FALSE/inconclusive (tighter gate made robust error worse). Re-running the source labs at VALIDATE caught it. Scoped the coupling to the unbounded-magnitude sims.

## Audit #34 (agent-memory-poisoning-corroboration-gate) — 2026-07-05 (REFRAME -> deployed)
- **VALIDATE (new public probe research/probes/poisoning_corroboration_gate.py):** reproduces the whole published table on the REAL shipped inspeximus, deterministically: ENTRENCH naive/default/hardened/sybil = 100/0/0/100; OVERWRITE = 100/100/0/100. The v1/v2 comprehensive labs' en_forged=0.00 (which flagged the post as wrong) was a LAB ARTIFACT: they forged the sybil with FAKE link strings (`links=["s0","s1"]`) that `_distinct_sources` resolves to 0 real records. A REALISTIC sybil (2 real distinct-source records the attacker mints) DOES graduate/overwrite -> 100%, so the post's "sybil=100%" is correct. Overwrite modeled via inspeximus's STATE-TOGGLE supersession (consolidate link_duplicates + value_clash pair) + `supersede_requires_corroboration`, NOT the `key=` path. BONUS cell: strict_corroboration + Ed25519 attestation -> two ATTACKER-HELD keys still clear the 2-witness gate (100%) = the boundary is PRICED (Douceur cost per witness), not shut at threshold 2.
- **THE stress-panel verdict = REFRAME (5/5 lenses damaging).** Prior-art: TEXTBOOK on both halves — Douceur 2002 (sybil bounded by resource-ratio rho absent a central authority), Cheng & Friedman 2005 (no non-trivial SYMMETRIC sybilproof reputation function; asymmetric max-flow CAN be), A-MemGuard 2510.02373 (consensus/corroboration memory-poisoning defense, >95% ASR cut). Ours = only the runnable N=150 confirmation on a shipped store. Skeptic+method: BOTH table endpoints are constructed (0% by the >=2 rule's definition, 100% by a `return True`/value>=5 naive strawman); "N=150" is theatre on a deterministic result; MINJA is query-only/self-authored so the gate doesn't engage MINJA's actual mechanism. Overclaim: "Measured"/"replicated the attack against inspeximus" borrow empirical credibility for a by-construction demonstration.
- **REFRAME applied EN+SK (all surfaces):** title dropped "Measured" -> "what it blocks, what it only prices"; TLDR + lead now say the 0% is "by construction, not a stochastic measurement" and the gate "prices, doesn't stop"; added a SCOPE paragraph ("we did NOT implement MINJA or ASB — cited as threat context; we tested the entrench/overwrite GOALS"); relabeled table col "naive store"->"naive (no guard)" + a bullet that the naive column is the definitional no-guard behaviour; added the "It prices, doesn't stop" section (Douceur + Cheng-Friedman callout; strict-attestation prices-not-closes callout cross-linking Veracity-Gap); added a "real frontier: independence, not counting" section (detect N-sources=1-actor; collusion floor -> per-actor outcome-accountability; Dissociative-Identity 2605.30169 FAccT-2026 for "attestation binds the container not the behaviour"); new FAQ ("Did you replicate MINJA/ASB?" No; "Is the 0% measurement or by construction?" By construction); rewrote falsifier to the boundary test; fixed the double-numbered <ol>; linked the public probe EN+SK+footer.
- **DEFECT 1 (citation, VERIFIED vs primary):** MINJA was cited as arXiv:2601.05504 -> FIXED to arXiv:2503.03704 (Dong et al., NeurIPS 2025, "Memory Injection Attacks on LLM Agents via Query-Only Interaction", 98.2% injection / 76.8% ASR). 2601.05504 is a DIFFERENT low-provenance Jan-2026 cs.CR follow-up (now cited NOWHERE). ASB "84.3% across 27 combinations" -> "highest average ASR 84.30%" (the 84.30% is a per-attack max-average, and "27" counts method TYPES not attackxdefense combos — verified vs 2410.02644, ICLR 2025).
- **Verify-subagent-citations caught a fabrication (again):** the stress prior-art hunter surfaced "Cordon-MAS arXiv:2605.26754 states 'resisted at k=2 (0% success)... raise to k=3' almost verbatim" — VERIFICATION found 2605.26754 is real but is a RAG information-flow-control defense with NO k-threshold quorum claim; the "verbatim" quote was hallucinated. NOT cited. Standing lesson reconfirmed: never cite a subagent-surfaced paper without primary-source verification.
- **Crucible ledger:** the post carries kicker="Crucible" but is NOT in server/.replications.json / crucible.json (73 entries) — same gap as #28. Did NOT auto-add (a public dataset entry is a new outward artifact) -> flagged to owner in the Slovak briefing for a decision (add an honest REPRODUCED-by-construction row, or drop the Crucible kicker).
- **FRONTIER Q (blind-spot lens, genuinely the next program):** corroboration-COUNTING audits the wrong axis (cardinality); every field that tried it (SybilLimit graph-cut, DMARC behavioural clustering, Wikipedia stylometric CheckUser) switched to detecting that N "independent" sources are secretly ONE actor. Can an agent-memory store estimate source-INDEPENDENCE under adversarial correlation without a central identity authority (Douceur says no in general) — and is genuinely-distinct COLLUSION the irreducible floor that forces the real lever to per-actor outcome-accountability (ties to [[membership-cost-vs-outcome-accountability]])?
- **Method lesson:** a "measured defense" table whose both ends are definitional (gate rule at 0, strawman baseline at 100) is a by-construction DEMONSTRATION — publish it as a runnable Crucible CONFIRMATION of the textbook bound (credit Douceur/Cheng-Friedman/A-MemGuard) on a real shipped artifact, never as "measured"/"a finding". And scope any MINJA/ASB citation: they are threat CONTEXT unless you implemented their actual attack (MINJA is query-only/self-authored — a corroboration gate addresses the durability/overwrite GOALS, not MINJA's transient-recall mechanism).

## Audit #35 (can-an-llm-trust-its-own-confidence) — 2026-07-05 (REFRAME -> deployed)
- **VALIDATE (recompute from raw per-item rows in research/probes/overconfidence_tax/*.json):** EVERY published number reproduces exactly. Arithmetic AUROC weak 0.50 / mid 0.538 / glm 0.73 / claude 0.903; overconf +0.72/+0.84/+0.19/+0.02; clean errors 51/66/16/23; risk-coverage (claude half 79% quarter 92% ans@90 44%; glm 93/100/63%). SimpleQA n=150 verbalized 0.47/0.74/0.74, multi-sample 0.57/0.63/0.71, quarter-acc 6/21/62%, base-acc 5/8/23%. Multisample arithmetic weak 0.972 / mid 0.98. Probe is public + linked (standing-gate OK). Added: Claude arithmetic 95% CI [0.81,0.99] (Hanley-McNeil, n=48, 25/23); weak conf 71/71 == exactly 1.0 (saturation); NO Claude-on-SimpleQA file exists (0.90 is arithmetic-only).
- **THE panel verdict = REFRAME (STORM 5 + stress 5, ~unanimous).** Prior-art: TEXTBOOK re-measurement. The finding (verbalized-confidence discrimination scales with capability; AUROC as the selective-prediction metric; weak~0.5) is Kadavath 2022 (2207.05221), Xiong 2023 (2306.13063, ICLR 2024 — the DIRECT prior: single-shot verbalized AUROC 0.51->0.63 GPT-3->GPT-4), Tian 2023 (2305.14975), Chow 1970 (reject option, IEEE TIT 16:41), Guo 2017 (1706.04599) + GPT-4 report (RLHF breaks calibration), Green&Swets 1966 (calibration vs discrimination). The post's "Honest prior art" section cited NOBODY -> now credits all, framed as re-measurement.
- **DEFECT A (title self-refutes):** title "...Only frontier" contradicted by the post's OWN SimpleQA table (mid 30B = frontier glm at 0.74). -> retitled "When can an agent trust its own confidence to abstain?"; body reframed to "verbalized single-shot confidence is a weak abstention signal that improves with capability but is task-dependent".
- **DEFECT B (the 0.90 conflates single-shot verbalized with sampling; academic):** Xiong's single-shot VERBALIZED frontier ceiling on real QA is ~0.63; 0.73-0.90 is SAMPLING/self-consistency territory. Our 0.90 is arithmetic-Claude best-case (computable task, confidence-saturated). -> added the ~0.63 single-shot-verbalized ceiling, scoped 0.90 to arithmetic + n=48 + CI + single anchor + "Claude not run on SimpleQA".
- **DEFECT C (a load-bearing claim was WRONG; economist + blind-spot):** post said "the value of an external verification/grounding gate is INVERSELY PROPORTIONAL TO MODEL CAPABILITY." A gate is INSURANCE priced on the cost/reversibility of a wrong action, NOT model IQ; at the frontier (higher-stakes, OOD tail where calibration degrades, cf. Kadavath P(IK) under shift) the gate can be MOST valuable. -> corrected to "gate value tracks reversibility and stakes first, capability only second". (Ties to [[membership-cost-vs-outcome-accountability]] + the reversibility-gate frontier.)
- **DEFECT D (saturation artifact; method auditor — sharpest):** weak/mid AUROC ~0.5 on arithmetic is partly DEGENERATE — those models emit ~1.0 on everything (weak 71/71 == 1.0), so AUROC=0.5 is the tie-convention readout, not "no latent uncertainty" — the post's OWN multisample data recovers 0.97 on the same weak model. -> foregrounded "partly a saturation artifact; a sampling signal recovers discrimination for the same models".
- **DEFECT E (base-rate confound; skeptic):** "only frontier gates to useful accuracy" partly restates "frontier is more accurate" (base acc 5/8/23%) — AUROC is base-rate-invariant, "useful-accuracy gating" is base-rate-dependent. -> the SimpleQA section already says "discrimination is not the same as usable abstention"; kept + the reframe no longer claims a clean capability law. Dropped "severe-tested" meta tag. Empty footer filled.
- **FRONTIER Q (blind-spot):** does frontier self-confidence stay discriminative on the OOD/adversarial hard tail (where it's deployed and where the cost is highest), or does "frontier-true" collapse exactly there? The honest next test = recompute AUROC + threshold-fixed selective-risk on an OOD split with stakes-weighted residual error. If it survives -> a scaling-relevant law; if it collapses -> the external gate is MOST needed at the frontier (inverts the naive reading).
- **Method lesson:** a home-grown small-n eval (n~80 arith / 150 SimpleQA, 3-4 models, one clean frontier anchor at n=48) that re-measures an established scaling result must (a) credit the specific prior art it re-measures (Kadavath/Xiong/Chow/Guo), (b) not let the TITLE assert what the harder benchmark refutes, (c) distinguish single-shot verbalized (~0.63 ceiling on QA) from sampling (0.73+), (d) never state an intuitive practical claim ("gate value ~ 1/capability") the economics contradict (gate = insurance on cost-of-error). Same "don't over-read a small clean number" family as the small-n AUROC CI.

## Audit #37 (food-nudges-publication-bias) — 2026-07-05 (REFRAME after a real data-check; thesis changed)
- **THE lesson (owner chose to do the data-check the skeptic demanded):** the ORIGINAL post's entire "food's 2.5x ranking IS a publication-bias artifact" rested on an UNVERIFIED empirical premise — that food-nudge studies are ~10x smaller than other-domain studies. STORM skeptic+academic+practitioner all flagged it was ASSUMED, not shown (cafeteria studies sometimes use large sales datasets -> could be reversed). We fetched Mertens 2021's own Table 1 (OSF osf.io/fywae). RESULT: premise SUPPORTED strongly — food IS by far the smallest-study domain (~113 participants/effect vs finance ~861 [7.6x], health ~1462, env ~1393, prosocial ~15782, other ~11345 [~100x]); the sim's 10x asymmetry is conservative. So the sim is a fair illustration -> REFRAME, not retract.
- **BUT the causal headline was WRONG and had to change:** Maier 2022 (the post's own cited support) found STRONG publication-bias evidence in EVERY subdomain (BF_pb>10) EXCEPT food (BF_pb~2.49, "moderate"/weakest). If food's lead were a funnel-asymmetry artifact, food should show the STRONGEST bias signal — it shows the WEAKEST. So "food's ranking is a proven publication-bias artifact" is unsupported-to-contradicted. Honest cause = small-n FRAGILITY + Type-M inflation (Egger 1997/Sterne 2011/Gelman-Carlin 2014), and food's tiny-n also gives funnel tests LOW POWER (BF~2.49 may mean "can't detect", not "no bias"). Retitled "...It's Publication Bias" -> "...Food Is the Small-Study Domain".
- **Citation fixes (verified vs primary):** Mertens never states "2.5x" — it's a derived food-vs-FINANCE ratio (food d=0.72 orig / 0.65 corrected vs finance d=0.24 -> ~2.7-2.9x); food is only ~1.5x the pooled mean (d=0.43). Added the Mertens CORRECTION (2204059119) the post omitted. Maier corrected pooled d=0.04 [0.00,0.14] (CI incl 0) — sharpened. Credited the textbook mechanism the post had NOT cited: Egger 1997 BMJ, Sterne 2011 (subgroup asymmetry != pub bias), Gelman-Carlin 2014 Type-M, + DellaVigna-Linos 2022 Econometrica (nudge-unit 1.4pp vs journal 8.7pp) + the outcome-cost/proximity confound (economist: ranking by raw d compares non-comparable outcomes — cheap proximal apple vs costly distal retirement).
- **VALIDATE + public probe:** promoted the gitignored lab (agora_output/lab/20260629-123411) to PUBLIC research/probes/nudge_pubbias_artifact.py (numpy, per-call fresh seed so each number is order-independent-reproducible, prints pooled 0.63/0.24 + ratio + control + full dose-response incl n=150). Reconciled post numbers to the probe: main 2.63->2.60x, control 1.00->1.01, sweep 150 1.30->1.28, 60 1.91->1.89, 30 2.64->2.60, 20 3.18->3.19. Linked EN+SK+footer.
- **The upgrade:** the audit turned a weakness (assumed premise) into a strength — the post now SHOWS the verified per-domain size table from Mertens's real data, and honestly discloses the Maier complication that keeps it from overclaiming. A REFRAME that ADDS rigor.
- **Method lesson (new, important):** when a null-result/debunk post rests on an empirical premise ("domain X studies are smaller/biased/etc."), VERIFY that premise in the target paper's own data BEFORE publishing — don't simulate the conditional and present it as the debunk. And never borrow a cited critique's authority (Maier) for a claim it doesn't make (Maier attacked the pooled effect; food's per-domain bias signal was weakest). The data-check is exactly the rigor the Crucible is for. Same "verify the load-bearing premise, not just the arithmetic" family as the small-n AUROC (#35) and the by-construction table (#34).

## Audit #38 (good-to-great-zero-skill-null) — 2026-07-05 (REFRAME -> deployed)
- **VALIDATE + public probe:** lab (agora_output/lab/20260629-141149) reproduces exactly; promoted to PUBLIC research/probes/good_to_great_null.py (numpy, N=1400~Collins' 1435 universe): 3x->30 firms/3.8x window/fwd +0.015/47% beat-mkt; 4x->15/4.7x/+0.041/48%; 5x->9(Collins found 11)/5.6x/-0.008/45%; sanity all-firms fwd 0.00000. BONUS the probe now quantifies the trait-retrofit (Texas-sharpshooter): at 5x cut ~1.8 of 60 candidate binary traits shared by ALL 9 firms by chance.
- **THE prior-art defect (same #37 pattern — verify whether it was already done):** the post claimed "What's new here is the runnable receipt: a zero-skill null." FALSE. Verified: a random-walk/chance null of Good to Great is ALREADY published — **Henderson, Raynor & Ahmed 2012, Strategic Management Journal, "Are 'Great' Companies Just Lucky?"** (formal significance test of G2G/BtL streaks vs chance; most named firms fail), **Denrell 2004 Management Science "Random Walks and Sustained Competitive Advantage"** (pure random walk -> persistent gaps + long streaks, the closest formal null), Denrell 2003 Org Science (undersampling of failure). Rosenzweig Halo Effect 2007 names Collins directly. -> reframed novelty claim to "a runnable re-demonstration of an established, peer-reviewed critique, not a discovery"; credited all four + Berk-Green.
- **The skeptic's real nuance (non-identifiability):** forward excess ~0 is ALSO the equilibrium prediction of a REAL-skill model — competition/capitalization competes skill away (Berk & Green 2004 JPE). So the null establishes NON-IDENTIFIABILITY (can't tell zero skill from real-but-competed-away skill), NOT "it was luck." Added an explicit "one honest subtlety" paragraph: the defensible verdict is "the evidence can't decide", the null kills the CAUSAL CLAIM the design couldn't support, not the possibility of skill. (Post was already mostly agnostic; sharpened.)
- **Citation care:** Kahneman's named example in Thinking Fast & Slow is BUILT TO LAST + In Search of Excellence, NOT Good to Great by name — so did NOT cite Kahneman as naming G2G (used Rosenzweig/Denrell as the G2G-specific critiques). Verified Collins: ~1435 universe, 11 firms, >=3x market/15y threshold, 6.9x average (added). Circuit City Ch.11 2009, Fannie Mae conservatorship 2008, Wells Fargo ~3.5M fake accounts/$185M CFPB 2016 (added), 11-stock portfolio underperformed S&P (Levitt 2008, added). Historical precedent added: In Search of Excellence (Peters-Waterman 1982) -> BusinessWeek "Who's Excellent Now?" 1984; Built to Last half-slipped; "Collins is his own ignored control group".
- **Method lesson (reinforces #37):** the recurring make-or-break on a debunk post is PRIOR ART + PREMISE — (1) has someone already done this exact null in a peer-reviewed venue? (Henderson-Raynor-Ahmed did) -> credit it, don't claim novelty; (2) does a symmetric read undercut the deflationary headline? (non-identifiability: zero-forward-alpha is also the skill-equilibrium) -> stay agnostic, never tip to "it was luck". Same family as #37 (verify the premise in the source) and #35 (don't over-read a clean number).

## Audit #39 (llm-as-judge-length-confound) — 2026-07-05 (THESIS REVERSED by our own falsifier; owner chose to run the control)
- **THE landmark: a published post's central causal claim was REFUTED by the very control it pre-registered as its falsifier.** Original thesis: "LLM-as-judge's ~80% human match is HALF just length" (a verbosity confound). STORM academic+skeptic both flagged the fatal gap: length CORRELATES with genuine quality (longer answers are often better on MT-Bench), so a length-only null reaching 68% is a CORRELATIONAL upper bound, NOT a causal decomposition — "recovers half the agreement" != "half is fake understanding". The one experiment that separates confound from valid-proxy is the length-MATCHED control, which the post PREDICTED would confirm it but NEVER RAN. Owner chose to run it (the #37 (A) pattern). RESULT (real MT-Bench, deterministic strict-majority): on length-matched pairs (<10% gap, n=74) GPT-4 still agrees with humans **79.7%** while the length-null falls to **0.530 (chance)**; matched<5% 87.8%; imbalanced>30% 89.5%. So the agreement SURVIVES length-matching -> largely SEMANTIC. By the post's own pre-registered falsifier ("if agreement stays ~80% on matched pairs while the null drops to chance, the verdict is wrong") the confound thesis is FALSIFIED. REVERSED the post: "we tried to debunk it as a length trick; our own control refuted it" (Campbell-Fiske 1959 shared-method-variance framing).
- **VALIDATE + real bug fixed:** the length-null numbers reproduce on the real data (char 68.1%, word 66.4%, vs-GPT4 73.5%). BUT the GPT-4-vs-human "majority" was NONDETERMINISTIC — max(set(votes),key=count) breaks split human votes randomly, wobbling agreement 0.838-0.865 run-to-run. Fixed to STRICT majority (drop tied pairs) -> stable 0.863. Promoted the gitignored lab to public research/probes/llm_judge_length_null.py (downloads the HF dataset) and EXTENDED it with the length-stratified control (the decisive evidence, now re-runnable). Linked EN+SK.
- **Citation fixes (verified vs primary):** Zheng 2023's real numbers are 85% GPT-4-human / 81% human-human (NON-TIE); 66%/63% with ties. The post's "80%"/"86.5%" -> reconciled to 85% (Zheng) and our reproduced 86.3%. Verbosity bias IS flagged in Zheng's own paper. Credited the family the post under-cited: Wang 2023 (2305.17926 position bias), Singhal 2023 (2310.03716 length-only reward reproduces most RLHF gains), Dubois 2024 (2404.04475 length-controlled AlpacaEval, 0.94->0.98, gameability 21%->6%), Saito 2023, + the 2026 fixed-rubric audit (verbosity <0.011). Novelty was overclaimed ("what's new is the runnable receipt") -> the phenomenon is textbook; only the specific MT-Bench null number + the control are ours.
- **The current-judge table (Claude/DeepSeek/GLM pick-longer ~72%) was DROPPED from the reversal:** it needed API keys (ANTHROPIC out of credits, can't re-verify), n~96, AND the skeptic showed it's inflated by construction (multijudge_length.py keeps only length-imbalanced human-favored-longer pairs, so "picks longer" and "agrees with human" are entangled). Not load-bearing for the reversed thesis.
- **Method lesson (the strongest of the series):** on a debunk post, the load-bearing risk is a CORRELATIONAL null presented as a CAUSAL confound. A null baseline "recovering X% of a signal" NEVER proves the signal is X% fake if the null variable correlates with the real thing — you MUST run the controlled/matched condition. Here the post even wrote that control as its falsifier and skipped it; running it reversed the verdict. Same family as #37 (verify the premise in the data) and #38 (prior art already did it), escalated: here the pre-registered falsifier itself was the unrun make-or-break. Also: check tie/majority handling is DETERMINISTIC before trusting an agreement number.

## Audit #40 (founder-led-survivorship-null) — 2026-07-05 (REFRAME: overclaim + ignored counter-evidence)
- **VALIDATE + public probe:** lab reproduces exactly (1.4x->1.55x/26%, 1.8x->2.60x/76%, 2.2x->4.77x/179%; median 1.58x). Promoted to public research/probes/founder_survivorship_null.py (numpy, deterministic; honestly labels the 1.8x volatility as an ASSUMPTION).
- **THE overclaim (same pattern as #37/#39):** the post claimed "MOSTLY survivorship / 76%" but the 76% rests entirely on an UNVERIFIED, hand-set 1.8x volatility assumption. VERIFIED: no source supports 1.8x; direction is ambiguous (younger/tech founder firms -> higher IVOL, BUT family/founder-CONTROLLED firms are repeatedly found MORE conservative / LESS volatile). The reproduction swings 26%->179% across the sweep -> "76%" is a researcher degree of freedom (curve-fit to the headline). ALSO the 1.8x coincidentally equals Bain's ex-tech RETURN multiple (don't conflate). -> reframed to conditional "survivorship CAN inflate it; the exact share is unmeasured".
- **THE counter-evidence the post ignored (CONFIRMED):** Fahlenbrach 2009 (JFQA 44(2):439-466) = a forward-formed, equal-weighted, delisting-inclusive founder-CEO portfolio (essentially the post's OWN unrun falsifier) earned +8.3%/yr benchmark-adjusted and **+4.4%/yr abnormal SURVIVES controls** (firm/CEO/industry). A controlled cross-sectional alpha, NOT an index artifact -> "it's just survivorship" is refuted at the CONDITIONAL level. Academic's key framing: the null and Fahlenbrach test DIFFERENT ESTIMANDS (null = unconditional raw 3.1x is uninformative; Fahlenbrach = conditional founder alpha exists). -> added Fahlenbrach prominently + the estimands distinction.
- **mean>>median is NOT a survivorship fingerprint:** all equity returns are right-skewed (Bessembinder 2018 -> a few stocks drive all wealth), so mean>>median is baseline for any buy-and-hold cohort -> softened from "the survivorship signature" to "consistent with, not diagnostic of".
- **Bain stat CONFIRMED survivorship-prone:** 1990-2014 founder-led S&P 500 = 3.1x the rest; built on CURRENT index membership (end-defined, look-ahead). So the raw-number critique is FAIR (kept). Credited textbook survivorship (Brown-Goetzmann-Ibbotson-Ross 1992 RFS; McLean-Pontiff 2016 anomaly decay; mutual-fund dead-fund bias).
- **Practitioner nuance added:** the premium is REAL but shrinking (Bain's own update: 2.1x since 2015, down from 3.1x) and NARROW/concentrated (Nvidia/Tesla/Meta); the Global X Founder-Run ETF (BOSS) was LIQUIDATED 2023 -> can't buy "founders" as a broad factor. So the honest picture = inflated raw number + a small, concentrated, decaying real premium (not mentality-magic, not pure artifact).
- **Method lesson (the recurring #37/#39/#40 pattern, now a documented 3-post family):** a debunk's fatal risk is (1) a load-bearing UNMEASURED premise chosen to hit the headline (here the 1.8x vol; #37 the size gap [that one checked out]; #39 the length-is-confound), and (2) IGNORING controlled counter-evidence that already ran the post's own falsifier (Fahlenbrach here; the length-matched control in #39). Before publishing a debunk: ground the load-bearing input, run/find the controlled version of the falsifier, and never let the null strawman a conditional claim. "Reproducing the raw ratio does not falsify the conditional effect."

## Audit #41 (chatbot-arena-style-not-skill) — 2026-07-05 (REFRAME to a TWO-LEVEL claim; the audit-series finale)
- **VALIDATE + public probe:** lab reproduces exactly on the real 140k Arena votes: style-only 61.5% (AUC 0.655), length-only 61.5%, chance 50.8%; leaderboard-order Spearman 0.748(51 models)/0.743(48)/0.732(44). Promoted to public research/probes/arena_style_only.py (downloads HF data; now also runs the within-pair control).
- **THE confound-vs-proxy trap (same as #39, applied correctly this time):** the original "ranks by style AS MUCH AS skill / 74% of the order is style" over-reads a CORRELATIONAL result — style co-moves with quality ACROSS models (better models write longer/better-formatted), so a style-only rank tracks the order because style is a valid PROXY, not because style IS what's ranked. Reproducing the order via a proxy is not a style-vs-skill decomposition.
- **I ran TWO decisive tests the post lacked, and they split the verdict:**
  (a) WITHIN-PAIR control (vote level): among battles between the SAME two models (quality fixed), the LONGER answer still wins ~62% (62.1% at >=20 battles/pair, 63.3% at >=50) -- essentially unchanged from the unconditional 61.5%. So unlike #39 (where the length-matched control COLLAPSED the effect), here the control CONFIRMS a genuine, quality-independent length bias in the VOTES. (Honest caveat: within-pair still has a second-order confound -- on prompts where the stronger side answers it may also be longer -- so it's a strong association, not a clean RCT.)
  (b) ORDER level = LMSYS's published style-CONTROLLED Elo (the post's own falsifier, already run by LMSYS): it reorders only MODESTLY -- top stays (GPT-4o/Claude/Gemini), Claude 3.5 Sonnet RISES 6->4, only formatting-leaning minis fall (GPT-4o-mini 6->11, Grok-2-mini 6->18). LMSYS explicitly warns of "positive correlation between length and substantive quality" and refuses to call it a pure confound. So the ORDER is mostly SKILL.
- **REFRAME to two levels:** "Arena VOTES carry a real length bias (test B), but the leaderboard ORDER is mostly skill (LMSYS style-control), not 'style as much as skill'." Retitled "...ranks by style as much as skill" -> "How much of Chatbot Arena is style? The votes are biased; the order mostly isn't." The within-pair control is a genuine ADDITION that strengthens the (narrower) vote claim.
- **Citation fixes (verified):** "The Leaderboard Illusion" (2504.20879, Singh-Hooker) is NOT about style -- it critiques private testing / unequal data access / score retraction. REMOVED (was a misattribution). Feuer "Style Outweighs Substance" (2409.15268, ICLR 2025) is about LLM JUDGES, not the human Arena -> scoped. Chiang 2024 (2403.04132, ICML) added: crowd agrees with experts (genuine capability signal). ρ=0.74 is NOT an LMSYS number (it's ours) -- clarified. LMSYS gives no "% of Elo that is style".
- **Method lesson (caps the 3-post confound-vs-proxy family #39/#40/#41):** "a null/proxy that RECOVERS X% of a signal" is a CORRELATIONAL CEILING, never a causal decomposition, whenever the null variable correlates with the real thing. To claim the signal IS the confound you MUST run the CONTROL: hold the real variable fixed (within-pair here, length-matched in #39) OR use the published controlled measure (LMSYS style-control, Fahlenbrach in #40). #39 the control REVERSED the claim; #41 the control SPLIT it (vote-confound real, order-proxy mostly skill); #40 the controlled study REFUTED the strong version. Always run the control before "it's mostly <confound>".
