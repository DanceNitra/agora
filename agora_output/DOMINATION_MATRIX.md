# inspeximus domination matrix — the integrity benchmark inspeximus WINS

> Consolidated from every measured result of the 2026-07-17/18 session. The claim is NOT "inspeximus beats everyone
> on one number" (on any single axis a competitor ties: fidelity ~ naive, forgetting ~ graphiti). The claim is
> **inspeximus is the ONLY system that passes EVERY integrity cell — every competitor fails at least one.** Each
...
| Integrity cell | probe | **inspeximus** | mem0 (live) | graphiti (live) | naive |
|---|---|---|---|---|---|
...
- **mem0** ❌ fidelity (0.16, 5× behind inspeximus) ❌ forgetting (leaks in history+raw) ❌ governance (2/8). Fails the most.
...
- **inspeximus** — the only row with no ❌.

## Honest caveats (do not trumpet past these)
- Fidelity: inspeximus ties naive verbatim (0.85 vs 0.87); the win is over the REAL competitor mem0 (5×), because mem0's
...
- Echo: LIVE mem0 on a BENIGN single echo is good (0.05, its LLM judge resolves it); the 1.0 figures for the
  "faithful" strategy are MODELS, not live mem0. inspeximus's echo_guard win is clean vs graphiti (0.72–0.81) and vs
...
1. **Fidelity: inspeximus 5× live mem0** (0.85 vs 0.16, reproduces mem0's published 0.18). Verified.
2. **Verifiable forgetting: inspeximus 1.0 vs live mem0 0.59** (mem0 cannot scrub its history DB + raw vectors → the
...
the full matrix in INTEGRITY_BENCHMARK.md with the harness next to every number. inspeximus stands as the only
