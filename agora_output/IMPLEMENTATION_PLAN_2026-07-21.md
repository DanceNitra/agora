...
Current state assumed: `agora-mnemo` **1.24.4**, CLI subcommands `browse consolidate contradictions
...
### 0.1 `mnemo install` — one command configures the host
...
Build `mnemo install --ide claude|cursor|codex|windsurf|cline` that writes the MCP server block into
that host's own config file and prints what it wrote.

- Files: `mnemo/cli.py` (new subcommand), new `mnemo/install.py` (per-host config paths + writers).
...
- **Acceptance:** on this machine, `mnemo install --ide claude` produces a config Claude Code actually
...
*Briefing item 1. Why: 39 threads — the single loudest ask — and `MnemoStore` already passes an
...
1. **`langchain-mnemo` package** — LangChain now requires third-party integrations to live in your own
   package. Thin wrapper re-exporting `mnemo.integrations.langgraph`, its own PyPI name, its own CI.
...
3. **LlamaIndex `BaseMemoryBlock`** and **Google ADK `BaseMemoryService`** — ADK's catalogue has little
   competition. `mnemo/integrations/llamaindex.py` and `google_adk.py` exist; the work is conforming to
   each framework's registration contract and getting listed.

- **Acceptance:** a clean machine can run `pip install langchain-mnemo`, paste six lines from the
...
*Briefing item 6. Why: 22 projects, and the capability already exists — `mnemo contradictions` is in
...
- Files: `mnemo/mnemo.py` ingest path, `benchmarks/memops/`.
...
### 3.2 `mnemo import --from mem0|zep` (item 8)
