...
- SKEPTIC: eager-vs-lazy materialized-view maintenance (Zhou VLDB'07), SQL DEFERRABLE constraints, TMS must
  ...
  against a self-chosen adversarial ordering — a CLOSED STRAWMAN (inspeximus's own primitives). A publishable
...
as a inspeximus feature, documented honestly against the prior art above (Graphiti bitemporal, lazy IVM, TMS/ATMS).
A correct, useful feature (inspeximus today has only the write-time one-shot cascade), NOT a novel result.

The genuinely-open slice IF we ever want research value: a purely EMPIRICAL cross-system escape-rate benchmark
on REAL agent stores (mem0/Graphiti/inspeximus) under realistic out-of-order delivery — but it largely reduces to
