...
```
/loop Resume Agora ops and KEEP PRODUCING overnight — do NOT idle until morning. FIRST read C:\Users\Danculus\agora\HANDOFF.md fully. Then EACH cycle do BOTH a health recheck AND one real unit of work. (A) HEALTH RECHECK (fast): verify the 5 processes alive — brain (agora.main, exactly one :8000 listener), dungeon (mcp_server.py), tools/dungeon_canary.py, tools/agent_activity_monitor.py, tools/self_improvement_controller.py — RELAUNCH any dead via HANDOFF.md §Relaunch; confirm the dungeon truly advances with `python tools/dungeon_health.py 20 3` (loop_n must move; HTTP 200 lies); confirm flags AGORA_SCIENTIST_LAB + AGORA_SCIENCE_GATE (server/.env) + AGORA_QUIET_GENERATORS (agora-game-server/.env). (B) PRODUCE ONE real unit this cycle, highest-value first: (1) any self-improvement-controller inbox task (AUTO-REBUILD / OPPORTUNITY SCAN / SELF-RESEARCH), one at a time, each verified; else (2) run a severe-tested research cycle that yields a Lab-backed finding — push a mechanism-explicit hypothesis through the severe-test path (real Methods Lab run + falsifier), or advance the Crucible by replicating an external claim where FAILED is a live possibility; else (3) advance the top pending item in HANDOFF.md (the `match`-organ ROI fix — flag-gated, py_compile, verify both servers 200, revert on breakage); else (4) frontier research on the Board priority, or draft a GATED distribution/outreach artifact for the owner's morning review. (C) Measure what you produced and Telegram ONE Slovak line naming the CONCRETE output (a finding + its Lab number, a replication verdict, a drafted artifact) — never just "all healthy". RULES: at most one substantive code change per cycle; never auto-post outreach/press (gated — leave drafts for morning approval); never ship an untested code change; verify measured numbers vs the source lab before any citation; change→observe→measure→next. Re-check AND produce every 25 minutes.
```
