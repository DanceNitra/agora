# Agora — Session Handoff (2026-06-25)

## ⏩ PASTE THIS AFTER SESSION RESTART (re-arms the 25-min dungeon/agent recheck loop)

```
/loop Resume Agora ops + production. FIRST read C:\Users\Danculus\agora\HANDOFF.md fully (the top section = 2026-06-25 state). EACH cycle do BOTH a health recheck AND one real unit. (A) HEALTH: verify the 5 processes alive — brain (agora.main, exactly one :8000 listener), dungeon (mcp_server.py), tools/dungeon_canary.py, tools/agent_activity_monitor.py, tools/self_improvement_controller.py — RELAUNCH any dead via HANDOFF.md §Relaunch; confirm the dungeon advances with `python tools/dungeon_health.py 20 3` (loop_n must move; HTTP 200 lies); confirm flags AGORA_SCIENTIST_LAB + AGORA_SCIENCE_GATE (server/.env) + AGORA_QUIET_GENERATORS (agora-game-server/.env). (B) DISTRIBUTION WATCH (live this week): run `python agora_output/distribution_radar/reddit_reply_watch.py` (it knows the 2 target threads and finds our comments itself) — on any NEW reply, draft a SLOVAK briefing (who replied / what they said / our proposed answer / how we use it) and WAIT for the owner — GATED, he posts manually, NEVER auto-reply. Account + thread/comment specifics are in the `distribution-experiment-live-reddit` memory (kept out of this public file). (C) PRODUCE ONE real unit, highest-value first: a self-improvement-controller inbox task (AUTO-REBUILD needs a brain restart -> do WITH owner); else drain ONE backlog research task from GET /brain/claude-inbox (read .pending) with the severe-test rule (real Lab run + falsifier; POST /brain/claude-inbox/done {id,result}) and clear a few clearly off-Board/saturated ones with skip-notes; else a fresh severe-tested Lab finding or a Crucible replication where FAILED is a live possibility. ALL experiments CLOUD-FREE (local nomic/qwen/llama + Claude as a model; NO ollama-cloud). (D) Telegram ONE Slovak line ONLY for a genuine breakthrough (major Lab finding, surprising FAILED, actionable artifact, breakage) — else stay silent but record. RULES: at most one substantive code change per cycle; never auto-post outreach/press (gated, Slovak briefing first); verify measured numbers vs the source before any citation; py_compile + both servers 200 + exactly one :8000 listener before commit; revert on breakage; small separate commits. Re-check AND produce ~every 25 minutes.
```

It's a dynamic `/loop` — I self-pace at ~1500s (25 min) and actively produce each cycle (research/severe-tests/replications/gated drafts), not just health-check. Telegram token is in `server/.env`; never echo it on a command line. Chat with the owner in **Slovak**; code + output **English**.

---

## 🟣🟣🟣 RESUME HERE (2026-06-25 — FRESHEST · the mnemo-forget + hybrid-sonda + distribution session) 🟣🟣🟣
