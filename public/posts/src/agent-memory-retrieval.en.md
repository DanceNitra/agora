...
**The falsifier.** If, on this same LoCoMo set, a single vector index with a strong embedder (run with correct prefixes, no reranker) beats BM25 at recall@20 with a conversation-level CI that excludes zero — or if a recency window reaches BM25-level recall — the core claims break. [The script](https://github.com/DanceNitra/agora/blob/main/research/probes/locomo_retrieval_map.py) and the raw per-method results are public, and the embeddings regenerate deterministically, so anyone can reproduce or refute this.
