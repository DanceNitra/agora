# Twelve concurrent writers, 9 records lost out of 2,880, every one of them reported as stored. Two causes we measured and refuted before the third one held, and a fourth that CI found after the fix shipped.

I ship an agent memory store whose pitch is that a write either lands or tells you it did not. For
twelve days in September its own CI said otherwise, and for most of those days nobody read it.
...
On 8 September the `tests` workflow had failed on 11 of its previous 12 runs. The one green run was
`81b042f`. The dominant cause was embarrassing and unrelated: one probe compared its count of clean
trials against a constant while the code above it dropped void trials, so a worker that failed to
start on a small runner read as a lost record, and the message printed the losses as `[0, 0, 0]`
next to the failure it was reporting. That was fixed in `e7a0d19`, and the pipeline went green for
two runs.
...
different reason. A chronically red pipeline is not a pipeline with many failures. It is a pipeline with no signal.
...
signature, and NTFS advances mtime in steps of 0.5 to 1.5 ms, so the collision is real. Measured over
1,500 same-length writes, `(mtime_ns, size)` collided 211 times; adding `st_ino` or a content hash
brought that to 0.

So the third arm widened the signature. It lost nothing, and neither did the arm that only fixed the
ordering. The collisions exist and are not the cause; the signature is unchanged in the release, and
the store pays nothing for a field it does not need.
...
difference that is not there. The caller gets a `StoreChangedOnDisk` it can retry. A false refusal is
recoverable; a silent overwrite is not.

## What the deterministic test looks like

A race that loses one record in 320 is not a test. The test that pins this monkeypatches the read
so a second handle writes inside it, then asserts that the record which landed inside the read is
...
everything. It fails on the old order every time and passes on the new order every time, which is
what a test is for. The probe stays beside it for the number.

## The third instance, found by CI after the fix shipped

The fix above went out as 2.27.1, and the notes for it warned that a fix lands at the reported
instance while the class survives. Then CI on a 2-vCPU runner lost `w7:r0` at commit `0a26545`, with
the lock held on every writer and the ordering already fixed. The class had survived one call site
over.

`reload()` is the retry path the `StoreChangedOnDisk` message tells a caller to use. It calls the
fixed `_load_from_disk`, merges the handle's own records back in, and then assigned a fresh signature
again: a stat after the read, outside the lock. A write landing in that window left the reloading
handle with records that predate the write under a signature that postdates it. The next save
rewrote the file from the stale view. Same mechanism, second call site.

Why the probe said 0: it retried by opening a new handle, which never enters `reload()`. It measured
a path that does not lose records. The CI harness retries the way the error message says to. Two
arms were added that retry through `reload()`, and the probe was run on Linux pinned to 2 CPUs, 80
rounds of 12 writers:
...
On Windows the same five arms over 30 rounds lost 0 on `reload-old`, which is why the desk never saw
it. The ext4 mtime tick on that box is about 4 ms and the race is about five times more frequent
there on the old ordering (50 against 5 per run). 2.27.5 removes the re-stamp, and `reload()` retries
its own merge and save, bounded at eight, so the recovery path does not hand back the exception it
exists to get past. The deterministic test for it uses a genuine second handle writing inside the
reload's read: it fails on 2.27.4, passes on 2.27.5, and restoring the re-stamp fails it again.
...
`tests/test_a_reload_that_restamps_after_the_read_repeats_the_defect.py`. The CI count is `gh run list --workflow tests` on that repository, the twelve
runs ending at `6eee2de`.
