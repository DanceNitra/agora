...
RUN (free, mnemo+naive):  python mnemo/probes/fair_conflict.py --systems mnemo,naive --n 40 --poison exact
RUN (with mem0):          python mnemo/probes/fair_conflict.py --systems mnemo,naive,mem0 --n 30 --poison paraphrase
"""
...
def sys_naive(subject, writes):
    m = Mnemo(path=None); m.echo_guard = False
