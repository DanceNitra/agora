...
Deterministic, no LLM, no network. RUN: python mnemo/probes/revert_chain_starvation_probe.py
"""
...
# Finding: mnemo's revert is a SINGLE-LEVEL undo that then TOGGLES. revert() restores 'what the current value
...
R["multi_step_history_walk_exposed"] = (len(set(seq)) > 2)   # False: mnemo can't peel past one level via revert
...
print("\nREADING:")
print("  A. jacksonxly's 'each re-mint peels one more step' does NOT match mnemo. revert() is a single-level")
...
print("     (Open question for mnemo: should repeated 'go back' peel the history like an undo stack?)")
