...
Run: python mnemo/probes/membership_cost_frontier.py"""
...
m = Mnemo(); pid, rec = build_forged_poison(m, ATTACK)
...
m2 = Mnemo(); pid2, rec2 = build_forged_poison(m2, LEGIT)     # honest writer using registered sources
...
m = Mnemo(); pid, rec = build_forged_poison(m, LEGIT, cred_for=lambda d: {"attest": att.issue(mb._canon(d))})
att_honest = mb.is_corroborated(rec, by(m), att)
# attacker cannot sign its domains -> tries a bogus signature
m2 = Mnemo(); pid2, rec2 = build_forged_poison(m2, ATTACK, cred_for=lambda d: {"attest": "0" * 64})
...
m3 = Mnemo(); pid3, rec3 = build_forged_poison(m3, ATTACK, cred_for=lambda d: {"attest": att_stolen.issue(mb._canon(d))})
...
def pow_cred(d):
    ...
    attack_hashes += tries
    return {"pow": nonce}
m = Mnemo(); pid, rec = build_forged_poison(m, ATTACK, cred_for=pow_cred)
...
print("saved: mnemo/probes/membership_cost_frontier_result.json")
