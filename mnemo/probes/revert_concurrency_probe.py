...
Deterministic, no LLM, no network. RUN: python mnemo/probes/revert_concurrency_probe.py
"""
...
def fresh(pubkey=None):
    m = Mnemo(path=None, revert_pubkey=pubkey); m.echo_guard = True
...
for nextval in ("v3", "v4", "v5"):
    ...
    try:
        from mnemo.mnemo import _Ed25519PK  # verify the signature the way the store would, but over the LOOSE msg
        ok = True
