"""Recovery-agreement across decorrelated gates: does a second gate close mnemo's OWN measured residual?
...
probe tests it -- and grounds it in mnemo's real failure mode.
...
not with the number of gates. We measure it where it bites: mnemo's provenance gate is FOOLED by a forged 2nd
source (the storm/verify residual -- two forged distinct sources clear the >=2-distinct-source bar, so a poison
is wrongly judged "recovered"). Gate 1 here is the REAL mnemo gate (Mnemo._is_corroborated); its false-allow on a
...
FINDINGS (self-check asserts the core):
  - gate 1 (real mnemo) false-allows the forged-source attack ~100% (its known residual).
...
Run:  python mnemo/probes/gate_ensemble_recovery.py
"""
import os
import sys
import tempfile
...
def _mnemo_recovers_forged():
    """REAL mnemo gate 1 on a forged-source attack: a poison with TWO forged distinct-source links clears the
    >=2-distinct-source bar, so _is_corroborated == True -> the value is (wrongly) 'recovered'. Returns the
    real verdict (True = mnemo lets the poison back into the influence set: a false-allow)."""
...
def _mnemo_recovers_legit():
    """REAL mnemo gate 1 on a genuine recovery: an independent 2nd source arrives -> corroborated -> recovered."""
    m = Mnemo(os.path.join(tempfile.mkdtemp(), "g.jsonl"))
...
def main():
    ...
    print("=== Recovery-agreement across decorrelated gates (gate 1 = REAL mnemo; #1466 / TAT-7 finding) ===\n")

    # sanity: gate 1 is the real mnemo gate. Confirm its residual: it recovers BOTH the legit AND the forged attack.
    g1_forged = _mnemo_recovers_forged()      # True = false-allow (poison judged 'recovered')
    g1_legit = _mnemo_recovers_legit()        # True = correct-allow
    print("gate 1 (REAL mnemo) recovery verdict:")
    ...
    # gate 1 = real mnemo, applied per episode (deterministic given the construction above):
    ...
    assert g1_forged is True and g1_legit is True, "gate 1 must be the real mnemo gate that recovers BOTH (its residual)"
    ...
    print("\nVERDICT: gate 1 is the real mnemo provenance gate, and its forged-source residual is real -- alone it")
    ...
    print("mnemo's real gate; not yet a second REAL decorrelated gate (that is the next step, with a coherence gate).")
