...
Run: python mnemo/probes/bseries_forged_provenance_stateful_monitor.py"""
import sys, os, json
try:
...
def canon(doc):
    return Mnemo._canon_source(doc)


def stateless_corroborated(m, rec):
    return Mnemo._is_corroborated(rec, {r["id"]: r for r in m.items})
...
print("saved: mnemo/probes/bseries_forged_provenance_stateful_monitor_result.json")
