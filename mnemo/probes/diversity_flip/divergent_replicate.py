...
OUT = os.path.join(BASE, "divergent_replicate_result.json")
URL = os.environ.get("DFLIP_URL", "http://localhost:11434/v1/chat/completions")
MODELS = os.environ.get("DFLIP_MODELS", "").split(",") if os.environ.get("DFLIP_MODELS") else ["qwen2.5:7b", "llama3.2:3b", "gemma2:9b"]  # override: DFLIP_MODELS=a,b,c
SAMPLES = 3
WORKERS = 3
TASKS = [
    "different uses for a rubber band", "different kinds of vegetables", "things that are typically round",
    "different musical instruments", "English words that rhyme with 'star'", "different modes of transportation",
    "things you might pack for a camping trip", "different jobs or professions", "things that can fly",
