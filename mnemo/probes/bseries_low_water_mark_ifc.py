...
Run: python mnemo/probes/bseries_low_water_mark_ifc.py"""
import sys, os, json
try:
...
def integrity(m, rec):
    return HIGH if Mnemo._is_corroborated(rec, by(m)) else LOW
...
def authority_allows(m, actor_id, blast):
    ...
    return Mnemo._is_corroborated(actor, by(m))
...
print("saved: mnemo/probes/bseries_low_water_mark_ifc_result.json")
