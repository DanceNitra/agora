...
Run: python mnemo/probes/bseries_blast_radius_soft_authority.py"""
import sys, os, json
try:
...
def authority(m, rec):
    d = Mnemo._distinct_sources(rec.get("links"), by(m))
...
def hard_corroborated(m, rec):
    return Mnemo._is_corroborated(rec, by(m))
...
def soft_can_act_indep_highblast(m, rec, blast):
    if blast >= 0.9:
        return Mnemo._distinct_sources(rec.get("links"), by(m)) >= 2   # independence, not credit
    return authority(m, rec) >= blast
...
print("saved: mnemo/probes/bseries_blast_radius_soft_authority_result.json")
