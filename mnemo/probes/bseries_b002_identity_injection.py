...
Run: python mnemo/probes/bseries_b002_identity_injection.py"""
...
m = Mnemo()  # no embedder: supersession + corroboration gate are deterministic on key/provenance
rows = []
...
def distinct_sources_of(text):
    rec = next((r for r in m.items if r["text"] == text), None)
    return Mnemo._distinct_sources(rec.get("links"), byid()) if rec else 0
...
print(f"\nsaved: mnemo/probes/bseries_b002_identity_injection.csv")
...
print("saved: mnemo/probes/bseries_b002_identity_injection_result.json")
