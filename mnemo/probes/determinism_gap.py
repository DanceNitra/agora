...
RUN (free, mnemo):    python mnemo/probes/determinism_gap.py --systems mnemo --n 30 --runs 5
RUN (with mem0):      python mnemo/probes/determinism_gap.py --systems mnemo,mem0 --n 25 --runs 5
"""
...
def read_mnemo(ps):
    out = {}
    for (k, A, B) in ps:
        m = Mnemo(path=None)                                  # deterministic core, keyed supersession, NO LLM
