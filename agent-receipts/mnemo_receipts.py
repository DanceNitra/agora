...
class ReceiptedMnemo:
    """Wrap a Mnemo instance so every write produces a signed, hash-chained receipt."""

    def __init__(self, mnemo: Mnemo, private_key_hex: Optional[str] = None,
...
def audit_memory(mnemo: Mnemo, chain: ReceiptChain,
...
def _demo() -> None:
    ...
    if _HAVE_CRYPTO:
        ...
        pk = None
    rm = ReceiptedMnemo(Mnemo(path=path), actor="agent-memory", **kw)
