"""What the perturbed-gasket within-orbit variance actually measures, settled by measurement.
...
THE QUESTION. If floor is comparable to rank2 at small eps, then most of his number is not the
symmetry breaking he intends to measure, and the advice in the letter has to say so.
...
def main():
    ...
    print()
    print("FINDING")
    print("  At eps = 0.001 the rank-2 statistic is %.4e and the floor is %.4e, a ratio of %.4f."
          % (r001["rank2"], floor, r001["rank2_over_floor"]))
    if r001["rank2_over_floor"] < 1.5:
        print("  So at small perturbation the statistic is dominated by the floor: it reports a")
        print("  property of picking one state out of a degenerate manifold, not the breaking.")
    else:
        print("  So the statistic is NOT dominated by the floor at small perturbation.")
    print("  The rank-4 statistic falls as eps^2 and is machine zero at eps = 0, so it has a real")
    print("  zero. It is the measure with a null; the rank-2 one does not have one.")
