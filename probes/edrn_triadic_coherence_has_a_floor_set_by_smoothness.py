"""What Marat's triadic coherence C reads when no shared mechanism is present.
...
A comparison like that has to answer one question first: what does C return on three streams that
share no mechanism at all? If three INDEPENDENT smooth curves already score in the 0.9s, the whole
observed range sits inside the null and the star is not separated from anything.
...
def main():
    ...
    print("FINDING: on independent streams C runs from %.4f at 4 modes to %.4f at 40 modes."
          % (smooth4["mean"], rough40["mean"]))
    print("         C tracks how smooth the three curves are. No shared mechanism appears")
    print("         anywhere in this construction.")
    ...
    print("The monotone arms are the ones that matter, because a real sweep is monotone:")
