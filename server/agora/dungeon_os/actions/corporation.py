...
async def _cto_evaluate_llm(title: str, summary: str, subsystem: str, enriched_prompt: str = "") -> dict:
    ...
    user_msg = (
        ...
        f"claims as minimal computational models and publish the verdict). Assess, HARDEST FIRST:\n"
        f"1. NOVELTY (not textbook): is this genuinely open, or a re-derivation of a KNOWN named result "
        f"(regression to the mean, collider/selection bias, survivorship, Simpson's paradox, majority-vote / "
        f"knowledge-conflict, knowledge-editing ripple effects, truth-maintenance / bitemporal invalidation)? "
        f"REJECT a textbook re-derivation even if it is perfectly testable — name the prior result it re-derives. "
        f"Our edge is out-EXECUTE (runnable receipts / replications), not out-discover a new 'law'.\n"
        f"2. TESTABILITY + NON-CIRCULAR MEASUREMENT: is there a simulable mechanism and a measurable quantity "
        f"(number, threshold, exponent, rate), measured NON-circularly — the metric and the outcome must not "
        f"share a mediator, and the ground truth must be EXTERNAL, not assigned by the same model that is judged?\n"
        f"3. GROUNDING: are the findings tied to a real named source, not vague generalities?\n"
        f"4. FALSIFIABILITY: could a minimal model genuinely come out either way (a falsifier that can actually fire)?\n"
        f"Approve only leads that are BOTH genuinely open AND cleanly measurable; reject textbook re-derivations, "
        f"vague/descriptive/unmeasurable ones, and circular or self-labeled measurements.\n\n"
        f"Respond with JSON:\n"
