...
async def _handle(app, text: str) -> None:
    ...
    if low.startswith(("fix ", "/fix ")):
    ...
    elif low in ("replications", "/replications", "rooke"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/replications")
        await send((d or {}).get("report", "_Rooke's bench is clean._"))
    elif low in ("map", "/map", "cartography", "wren"):
    ...
    elif low in ("roadmap", "/roadmap", "aldric", "panel"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/roadmap-inputs")
        await send((d or {}).get("report", "_No roadmap data._"))
    elif low in ("press", "/press", "posts"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/press")
        await send((d or {}).get("report", "_The press is quiet._"))
    elif low in ("scout", "/scout", "opportunities"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/scout")
        await send((d or {}).get("report", "_No opportunities logged._"))
    elif low in ("envoy", "/envoy", "engagement", "replies"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/envoy")
        await send((d or {}).get("report", "_The Envoy is watching no threads yet._"))
    elif low in ("portfolio", "/portfolio", "track", "track record"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/portfolio")
        await send((d or {}).get("report", "_No track record yet._"))
    elif low in ("academy", "/academy", "school"):
        d = await asyncio.to_thread(_brain_get, "/api/v1/agent-os/brain/academy")
        await send((d or {}).get("report", "_The academy is empty._"))
        ...
        elif a.get("kind") in ("export_insights", "digest", "curate", "gist", "publish", "outreach", "press", "portfolio", "distribute"):
